export type CreatorPlanKey = 'FREE' | 'PRO' | 'STUDIO'

export const CREATOR_PLAN_CONFIG = {
  FREE: {
    label: 'Free',
    price: '$0/mo',
    description: 'Up to 3 Trip Kits and 5 imported videos',
    maxImportedVlogs: 5,
  },
  PRO: {
    label: 'Pro',
    price: '$49/mo',
    description: 'Up to 25 imported videos, AI scan, and advanced analytics',
    maxImportedVlogs: 25,
  },
  STUDIO: {
    label: 'Studio',
    price: '$199/mo',
    description: 'Up to 100 imported videos, team seats, and premium support',
    maxImportedVlogs: 100,
  },
} as const satisfies Record<CreatorPlanKey, {
  label: string
  price: string
  description: string
  maxImportedVlogs: number
}>

export function getCreatorPlanConfig(plan: string | null | undefined) {
  if (plan === 'PRO' || plan === 'STUDIO') return CREATOR_PLAN_CONFIG[plan]
  return CREATOR_PLAN_CONFIG.FREE
}
