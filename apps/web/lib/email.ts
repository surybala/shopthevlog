...
const FROM  = () => `TripMirror <${process.env.ZOHO_SMTP_USER ?? 'cherry@vlogshopper.com'}>`
...
export async function sendWaitlistConfirmation(to: string, name: string) {
  return transporter().sendMail({
    from: FROM(),
    to,
    subject: "You're on the TripMirror waitlist 🎬",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;color:#111">
        <h2 style="margin-bottom:4px">Hey ${name} 👋</h2>
        <p style="color:#555">
          Thanks for requesting early access to <strong>TripMirror</strong>.
        ...
        <p style="color:#999;font-size:12px">TripMirror · Creator-first travel commerce</p>
      </div>
    `,
  })
}
...
export async function sendApprovalEmail(to: string, name: string) {
  ...
  return transporter().sendMail({
    from: FROM(),
    to,
    subject: "You're in — TripMirror early access ✅",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;color:#111">
        <h2>You're approved, ${name}! 🎉</h2>
        <p style="color:#555">
          Your early access to <strong>TripMirror</strong> is ready.
