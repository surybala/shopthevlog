  ...
  return (
    ...
    >
      <div className="mb-10">
        ...
        <h1 className="creator portal-heading text-3xl font-bold">Shop</h1>
        <p className="creator portal-muted mt-1">{creator.merchandise.length} products</p>
      </div>

      {creator.merchandise.length === 0 ? (
        <div className="creator portal-muted py-24 text-center">No products yet.</div>
      ) : (
        <div className="space-y-12">
          {physicalItems.length > 0 && (
            <section>
              <h2 className="creator portal-heading mb-4 text-lg font-semibold">Merch</h2>
          ...
          {digitalItems.length > 0 && (
            <section>
              <h2 className="creator portal-heading mb-4 text-lg font-semibold">Digital Downloads</h2>
              <div className="grid grid-cols-4 gap-4">
          ...
          {affiliateItems.length > 0 && (
            <section>
              <h2 className="creator portal-heading mb-4 text-lg font-semibold">Gear Picks</h2>
              <div className="grid grid-cols-4 gap-4">
...
function MerchCard({
  ...
  return (
    <Link href={`/@${handle}/shop/${item.slug}`} className="creator portal-card group overflow-hidden">
      <div className="creator portal-surface aspect-square overflow-hidden border-0">
      ...
      <div className="p-4">
        <p className="creator portal-heading truncate text-sm font-medium">{item.title}</p>
        <div className="mt-1 flex items-center justify-between">
          <p className="creator portal-muted text-xs">
            {item.type === 'AFFILIATE' ? 'Affiliate' : item.price !== null ? `$${(item.price / 100).toFixed(0)}` : 'Free'}
          </p>
          <p className="creator portal-muted text-xs capitalize">{item.type.toLowerCase()}</p>
        </div>
      </div>
    </Link>
  )
