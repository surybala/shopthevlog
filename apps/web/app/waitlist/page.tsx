...
// ── Inner form — needs Suspense because it reads searchParams ─────────────────
...
function WaitlistForm() {
  ...
  // If we arrived from the signup page with pre-filled data, show a hint
    ...
    <div className="w-full max-w-md space-y-8">
      ...
      {submitted ? (
        ...
        <form onSubmit={handleSubmit} className="glass-card p-8 space-y-4">
          {prefilled && (
            <p className="text-white/60 text-xs leading-relaxed bg-white/5 rounded-lg px-3 py-2 border border-white/10">
              Your details are pre-filled from the signup page — add an optional note and hit the button below.
            </p>
          )}
          <input
            type="text"
...
// ── Page shell — wraps WaitlistForm in Suspense (required for useSearchParams) ─
        ...
        fallback={
          <div className="w-full max-w-md space-y-8 animate-pulse">
            <div className="text-center space-y-4">
              ...
              <div className="h-6 bg-white/10 rounded w-56 mx-auto" />
            </div>
